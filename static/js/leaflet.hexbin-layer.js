L.HexLayer = L.Class.extend({
    ...
    _update: function () {
        ...
        if (!(zoom in this._levels)) {
            ...
            this._createNames(this._levels[zoom]);
    ...
    _createNames: function (container) {
        var layout = this._layout,
            data = this._data.features.map(function (d) {
                return this._projectNames(d.geometry_name);
            }, this),
            // bins = layout(data),
            hexagons = container.selectAll(".hexagon").data(data);

        // Create hexagon elements when data is added.
        var path = hexagons.enter().append("p");
        // this._applyStyle(path);

        // Position hexagon elements.
        hexagons.text( function (d) {
            // Setting "M" ensures each hexagon is drawn at its correct location.
            return "lol" + d;
        });
    },
